...
<p align="center">
  <a href="https://spcs.canterbury.ac.nz/mjo/telescopes.html">
    <img src="https://spcs.canterbury.ac.nz/mjo/images/boller_chivens-opt.jpg" width="760" alt="Boller &amp; Chivens 0.6m telescope at Ōtehīwai Mt John Observatory">
  </a>
</p>
...
The telescope motors, hand paddles, safety limits, dome motor outputs, and ST-4/SBIG autoguider inputs connect to the motion controller hardware rather than separately to the PC.
...
2. Log out and back in before testing the new group membership, then install the controller udev rule:
...
1. Copy the dump to `/home/observer/teljoy_latest.sql`.
...
1. Run the CGI programs using a dedicated account rather than `apache` or `observer`:
...
In a second terminal, restart and check Apache:
...
Both requests should return HTTP 200. Open `http://127.0.0.1/cgi-bin/bncstat` in a web browser and confirm that it shows live telescope data rather than the **Can't connect - is Teljoy running?** message.
...
This shortcut starts the correct interpreter in interactive mode and leaves a shell open after Teljoy shuts down:
